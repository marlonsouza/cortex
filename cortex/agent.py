...
CONTEXT7_ENABLED = get_config("CONTEXT7_ENABLED", "false", env_vars).lower() in ["true", "1", "yes", "y"]
...
CONTEXT7_LIBS = get_config("CONTEXT7_LIBS", "", env_vars)  # Comma-separated list of libraries
...
if CONTEXT7_ENABLED:
    ...
    if start_context7_server():
        ...
        print("Warning: Failed to start Context7 server, falling back to direct LLM connection")
...
# MCP client class
class MCPClient:
    def __init__(self, model, api_url=MCP_API_URL, api_key=MCP_API_KEY, 
                 temperature=MCP_TEMPERATURE, max_tokens=MCP_MAX_TOKENS, timeout=MCP_TIMEOUT):
        self.model = model
        ...
        if self.using_context7 and CONTEXT7_LIBS:
            self.context7_libs = [lib.strip() for lib in CONTEXT7_LIBS.split(',') if lib.strip()]
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            for msg in messages:
                if hasattr(msg, "role"):
                    ...
                    role = "system" if getattr(msg, "__class__", None) and msg.__class__.__name__ == "SystemMessage" else "user"
            ...
            if self.is_ollama:
                ...
                if "localhost" in base_url or "127.0.0.1" in base_url:
                    try:
                        url_parts = base_url.split("://")[1] if "://" in base_url else base_url
            ...
            logging.info(f"Debug - API URL: {self.api_url}, Is Ollama: {self.is_ollama}")
            ...
            for current_endpoint in endpoints_to_try:
                ...
                while retry_count < max_retries:
                    try:
                        logging.info(f"Attempt {retry_count+1} with endpoint: {current_endpoint}")
                        async with session.post(
                            ...
                            if response.status == 200:
                                ...
                                logging.info(f"✅ Success! Received response ({len(str(result))} chars) from: {current_endpoint}")
                        ...
                        error_msg = f"Connection error with {current_endpoint}: {str(e)}"
                        logging.warning(error_msg)
                        error_messages.append(error_msg)
                        retry_count += 1
                        ...
                        error_msg = f"Unexpected error with {current_endpoint}: {str(e)}"
                        logging.warning(error_msg)
                        error_messages.append(error_msg)
                        retry_count += 1
                if 'result' in locals():
            ...
            logging.info(f"Received response from API with {len(str(result))} characters")
            class MCPResponse:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Cortex - AI assistant to scaffold projects using MCP")
    parser.add_argument("--model", type=str, help=f"Model to use (default: {MCP_MODEL})")
    ...
    parser.add_argument("--temperature", type=float, help=f"Temperature (default: {MCP_TEMPERATURE})")
    parser.add_argument("--max-tokens", type=int, help=f"Max tokens (default: {MCP_MAX_TOKENS})")
    parser.add_argument("--project", type=str, help="Project name (skips the prompt)")
    parser.add_argument("--prompt-file", type=str, help="File containing the prompt (skips the prompt input)")
    ...
    if args.prompt_file:
        ...
        print("Enter your prompt for the assistant. Finish with EOF (Ctrl-D on Unix, Ctrl-Z+Enter on Windows):")
        import sys
    ...
    system_msg = SystemMessage(content=
        "You are an AI assistant that generates code and project structures based on user requirements. "
    ...
    async def _run_plan():
        try:
            ...
            logging.error("Please check your model configuration and network connection")
            if DEBUG:
                import traceback
        ...
        try:
            ...
            if content_match:
                ...
                if '.go' in json_str:
                    ...
                    content = re.sub(r'map\[string\]string\{([^}]+)\}', r'map[string]string{\1}', content)
                
                # Replace the content in the JSON
                json_str = re.sub(r'"content"\s*:\s*"(.*?)(?:"\s*}|\s*$)', 
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                debug_print("Fixed JSON snippet (first 100 chars):", fixed_snippet[:100] + "..." if len(fixed_snippet) > 100 else fixed_snippet)
    ...
    if plan is None:
        ...
        try:
            import re
            print("Approach 1: Attempting manual extraction of tool calls from JSON-like text...")
            # Extract make_directory call
            dir_match = re.search(r'"name":\s*"make_directory".*?"path":\s*"([^"]+)"', text, re.DOTALL)
            # Extract write_file call
            file_match = re.search(r'"name":\s*"write_file".*?"path":\s*"([^"]+)"', text, re.DOTALL)
            # Extract content (this is tricky)
            content_match = re.search(r'"content":\s*"(.+?)(?:"\s*\}\s*\}|"\s*\})', text, re.DOTALL)
            ...
            if dir_match and file_match and content_match:
                ...
                print("⚠️ JSON parsing issues detected, but proceeding with extracted tool calls.")
            ...
            # If the above doesn't work, try to detect and convert code examples
                ...
                plan = {
                    "tool_calls": [
                        ...
                        {"name": "write_file", "arguments": {"path": "rest-api/main.go", "content": "// This is a placeholder for the Go code"}},
                        {"name": "read_file", "arguments": {"path": "rest-api/main.go"}},
                        {"name": "delete_file", "arguments": {"path": "rest-api/main.go"}}
                    ]
                }
